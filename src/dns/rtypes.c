...
#include "rtypes.h"
#include "strutils.h"
#include "dns_packet.h"     // TODO : decrease these dependency.
...
// Note: The NULL in the qname field is important, and used to distinguish
...
void *A(const uint8_t *packet, uint32_t pos, uint32_t i,
...
void *domain_name(const uint8_t *packet, uint32_t pos, uint32_t id_pos,
                  uint16_t rdlength, uint32_t plen) {
    char *name = read_rr_name(packet, plen, id_pos, &pos);
    if (name == NULL)
        name = mk_error("Bad DNS qname: ", packet, pos, rdlength);
...
void *soa(const uint8_t *packet, uint32_t pos, uint32_t id_pos,
          uint16_t rdlength, uint32_t plen) {
    char *mname;
    char *rname;
    char *buffer;
    ...
    mname = read_rr_name(packet, plen, id_pos, &pos);
    if (mname == NULL) return mk_error("Bad SOA: ", packet, pos, rdlength);
    rname = read_rr_name(packet, plen, id_pos, &pos);
    ...
    for (i = 0; i < 4; i++) {
        serial <<= 8;
        serial |= packet[pos + (i + (4 * 0))];
        refresh <<= 8;
        refresh |= packet[pos + (i + (4 * 1))];
        retry <<= 8;
        retry |= packet[pos + (i + (4 * 2))];
        expire <<= 8;
        expire |= packet[pos + (i + (4 * 3))];
        minimum <<= 8;
        minimum |= packet[pos + (i + (4 * 4))];
    }
...
void *mx(const uint8_t *packet, uint32_t pos, uint32_t id_pos,
         uint16_t rdlength, uint32_t plen) {
    ...
    pos = pos + 2;
    name = read_rr_name(packet, plen, id_pos, &pos);
    if (name == NULL)
...
"(changed by bdpothik)\n"\
"Changed according to RFC 6891\n"

void *opts(const uint8_t *packet, uint32_t pos, uint32_t id_pos,
           uint16_t rdlength, uint32_t plen) {
    void *rdata;
    if (rdlength < 4) {
        char *buffer = escape_data(packet, pos, pos + rdlength);
        rdata = malloc(sizeof(char) * (strlen(buffer) + 1));
        sprintf(rdata, "%s", buffer);
        free(buffer);
    }
    else {
        dns_opt_data *lrdata = malloc(sizeof(dns_opt_data));
        lrdata->opt_code = (packet[pos] << 8) + packet[pos + 1];
        lrdata->opt_length = (packet[pos + 2] << 8) + packet[pos + 3];

        if (lrdata->opt_length >= 4 && lrdata->opt_code == 8) {
            lrdata->opt_data = malloc(sizeof(dns_ecs_data));
            dns_ecs_data *opt_data = (dns_ecs_data *) lrdata->opt_data;
            opt_data->family = (packet[pos + 4] << 8) + packet[pos + 5];
            opt_data->source_pre_len = packet[pos + 6];
            opt_data->scope_pre_len = packet[pos + 7];
            int addr_len = lrdata->opt_length - 4;
            if (addr_len > 0) {
                opt_data->address = malloc(sizeof(char) * addr_len);
                memcpy(opt_data->address, &packet[pos + 8], addr_len);
            }
            else {
                opt_data->address = NULL;
            }
        }
        else {
            lrdata->opt_data = malloc(sizeof(char) * (lrdata->opt_length + 1));
            char *opt_data = (char *) lrdata->opt_data;
            if (lrdata->opt_length > 0) {
                memcpy(opt_data, &packet[pos + 4], lrdata->opt_length);
            }
            opt_data[lrdata->opt_length] = '\0';
        }
        rdata = lrdata;
    }
...
void *srv(const uint8_t *packet, uint32_t pos, uint32_t id_pos,
          uint16_t rdlength, uint32_t plen) {
    ...
    target = read_rr_name(packet, pos + rdlength - 6, id_pos, &pos);
    if (target == NULL)
...
void *AAAA(const uint8_t *packet, uint32_t pos, uint32_t id_pos,
           uint16_t rdlength, uint32_t plen) {
    char *buffer;
    uint16_t ipv6[8];
    int i;
...
void *dnskey(const uint8_t *packet, uint32_t pos, uint32_t id_pos,
             uint16_t rdlength, uint32_t plen) {
...
void *rrsig(const uint8_t *packet, uint32_t pos, uint32_t id_pos,
            uint16_t rdlength, uint32_t plen) {
    uint32_t o_pos = pos;
    uint16_t tc = (packet[pos] << 8) + packet[pos + 1];
    uint8_t alg = packet[pos + 2];
    uint8_t labels = packet[pos + 3];
    uint32_t ottl, sig_exp, sig_inc;
    ...
    sig_inc = (packet[pos] << 24) + (packet[pos + 1] << 16) +
              (packet[pos + 2] << 8) + packet[pos + 3];
    pos = pos + 6;
    signer = read_rr_name(packet, o_pos + rdlength, id_pos, &pos);
    if (signer == NULL)
        return mk_error("Bad Signer qname", packet, pos, rdlength);
...
void *nsec(const uint8_t *packet, uint32_t pos, uint32_t id_pos,
           uint16_t rdlength, uint32_t plen) {

    char *buffer, *domain, *bitmap;

    domain = read_rr_name(packet, pos + rdlength, id_pos, &pos);
    if (domain == NULL)
...
void *ds(const uint8_t *packet, uint32_t pos, uint32_t id_pos, uint16_t rdlength, uint32_t plen) {
...
void *escape(const uint8_t *packet, uint32_t pos, uint32_t i,
             uint16_t rdlength, uint32_t plen) {
...
void print_parsers() {
    ...
    printf("What follows is a list of handled DNS classes and resource \n"
                   ...
                   "class, rtype, qname: documentation\n");
