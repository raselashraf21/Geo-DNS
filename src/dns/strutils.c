...
char *read_rr_name(const uint8_t *packet, uint32_t len, uint32_t id_pos, uint32_t *packet_p) {
    ...
    // Scan through the qname, one character at a time. We need to look at
    ...
    // for a compression jump or qname end.
    // It's possible that there are endless loops in the qname. Our protection
    // against this is to make sure we don't read more bytes in this process
    // than twice the length of the rdata.  Names that take that many steps to
    ...
    while (pos < len && !(next == pos && packet[pos] == 0)
        ...
        if (next == pos) {
            ...
            // to the start of the remainder of the qname.
            if ((c & 0xc0) == 0xc0) {
    ...
    // Due to the nature of DNS qname compression, it's possible to get a
    // qname that is infinitely long. Return an error in that case.
    // We use the len of the packet as the limit, because it shouldn't 
    // be possible for the qname to be that long.
...
int main() {
    ...
    pos = 29;
    s = read_rr_name(name_data, 41, 4, &pos);

    if ((strcmp(s, name_result) == 0) && pos == 41) 
        printf("qname parse test ok.\n");
    else {
        printf("pos: %d\n", pos);
        printf("qname parse test failed: \n%s\n%s\n", s,name_result);
        for (i=0; i<35; i++)
            printf("%x, %x, %c, %c\n", s[i], name_result[i],
                                       s[i], name_result[i]);
    }
